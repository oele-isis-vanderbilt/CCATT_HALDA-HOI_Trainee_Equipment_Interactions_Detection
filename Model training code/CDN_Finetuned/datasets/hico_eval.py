...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        for index, img_preds in enumerate(preds):
            # print("Debugging check program hico_eval line 27 preds",preds)
            # try:
            # img_preds = {k: v.to('cpu').numpy() if v is not None else None  for k, v in img_preds.items()}
            # except:
            #     print(k, v)
            # img_preds_new = {}
            # for k, v in img_preds.items():
            #     try:
            #         img_preds_new[k] = v.to('cpu').numpy() if v is not None else None
            #     except Exception as e:
            #         print(f"Error processing key={k}, value={v}, error={e}")
            import torch
            img_preds_new = {}
            for k, v in img_preds.items():
                if isinstance(v, torch.Tensor):
                    img_preds_new[k] = v.to('cpu').numpy()
                else:
                    # Keep string (or any other non-tensor) as-is
                    img_preds_new[k] = v
        
            img_preds = img_preds_new
    ...
    def compute_map(self):
        ...
        for triplet in self.gt_triplets:
            ...
            rec = tp / sum_gts
            prec = tp / (fp + tp)
            print("Debugging check program hico_eval line 161 ")
            print('Triplet: {}  AP: {}  max recall: {}'.format(triplet, rec, prec, np.amax(rec)))
            # exit()
