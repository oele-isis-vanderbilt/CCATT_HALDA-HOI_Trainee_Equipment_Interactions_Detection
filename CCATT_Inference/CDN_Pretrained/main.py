...
def main(args):
    ...
    # dataset_train / data_loader_train are only reachable from the training
    # loop further below, which `if args.eval: ... return` always skips
    # (confirmed by reading main.py's control flow) -- building the train
    # split (and requiring a non-empty annotations/trainval_hico.json) is
    # unnecessary when only running --eval. Skip it in that case.
    if not args.eval:
